...
class BasicTrainer(object):
    ...
    def shard(self):
        ...
        wrap_class = get_block_class_from_model(
            self.policy.pretrained_model if self.config.loss.name == 'ppo' else self.policy,
        ...
        mp_dtype = getattr(torch,
                           self.config.model.fsdp_policy_mp) if self.config.model.fsdp_policy_mp is not None else None
        ...
        if self.config.loss.name == 'ppo':
            self.policy.pretrained_model = FSDP(self.policy.pretrained_model, **shared_fsdp_kwargs,
        ...
        if self.config.model.activation_checkpointing:
            ...
            try:
                ...
                if self.config.loss.name == 'ppo':
                    apply_activation_checkpointing(self.policy.pretrained_model,
                                                   checkpoint_wrapper_fn=checkpoint_wrapper, check_fn=check_fn)
                else:
                    apply_activation_checkpointing(self.policy, checkpoint_wrapper_fn=checkpoint_wrapper,
                                                   check_fn=check_fn)

                if self.reference_model is not None:
                    apply_activation_checkpointing(self.reference_model, checkpoint_wrapper_fn=checkpoint_wrapper,
                                                   check_fn=check_fn)
    ...
    def get_batch_samples(self, batch: Dict[str, torch.LongTensor]) -> Tuple[str, str]:
        ...
        ctx = lambda: (FSDP.summon_full_params(self.policy, writeback=False,
                                               recurse=False) if self.fsdp else contextlib.nullcontext())
        with ctx():
    ...
    def get_batch_metrics(self, batch: Dict[str, Union[List, torch.LongTensor]], mode: str = None) -> Tuple[
    ...
    def train(self):
        ...
        for batch in self.train_iterator:
            ...
            if gradients_accumulated == 0 and (
                    last_log is None or time.time() - last_log > self.config.minimum_log_interval_secs):
                mean_train_metrics = {}
    ...
    def save(self, output_dir: Optional[str] = None, metrics: Optional[Dict] = None, save_model_only: bool = True):
        ...
        if self.fsdp:
            ...
            if not save_model_only:
                ...
                if self.rank == 0:
                    self.write_state_dict(self.example_counter, optimizer_state_dict, metrics, 'optimizer.pt',
                                          output_dir)
                ...
                if self.rank == 0:
                    ...
                    self.write_state_dict(self.example_counter, scheduler_state_dict, metrics, 'scheduler.pt',
                                          output_dir)
...
class UnpairedPreferenceTrainer(BasicTrainer):
    ...
    def forward(self, model: nn.Module, batch: Dict[str, Union[List, torch.LongTensor]], average_log_prob=False) -> \
            Tuple[torch.FloatTensor, torch.FloatTensor, torch.FloatTensor, torch.BoolTensor]:
        ...
        """
        all_logits = model(batch['target_combined_input_ids'], attention_mask=batch['target_combined_attention_mask'],
                           use_cache=(not self.is_mistral)).logits.to(self.policy_dtype)
    ...
    def get_batch_metrics(self, batch: Dict[str, Union[List, torch.LongTensor]], mode: str = None):
        ...
        metrics[f'rewards_{mode}/margins'] = [(all_rewards[chosen_rewards_idx].mean().nan_to_num(0) - all_rewards[
            rejected_rewards_idx].mean().nan_to_num(0)).item()]
        metrics[f'loss/{mode}'] = all_devices_losses.float().cpu().numpy().tolist()
...
class PairedPreferenceTrainer(BasicTrainer):
    ...
    def forward(self, model: nn.Module, batch: Dict[str, Union[List, torch.LongTensor]], average_log_prob=False) -> \
        ...
        all_logits = model(concatenated_batch['concatenated_combined_input_ids'],
...
class KT_IPATrainer(UnpairedPreferenceTrainer):
    def loss(self,
             policy_chosen_logps: torch.FloatTensor,
             policy_rejected_logps: torch.FloatTensor,
        ...
        losses = torch.cat(
            (self.config.loss.desirable_weight * chosen_losses, self.config.loss.undesirable_weight * rejected_losses),
            0)
    ...
    def forward(self, model: nn.Module, batch: Dict[str, Union[List, torch.LongTensor]], average_log_prob=False) -> \
        ...
        all_logps = get_batch_logps(all_logits, concatenated_batch[f'concatenated_labels'],
                                    average_log_prob=average_log_prob)
    ...
    def get_batch_metrics(self, batch: Dict[str, Union[List, torch.LongTensor]], mode: str = None):
        ...
        with torch.no_grad():
            reference_chosen_logps, reference_rejected_logps, reference_KL_logps = self.forward(self.reference_model,
                                                                                                batch)
